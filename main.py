...
# Close cursor and connection after creating the database
cursor.close()
db_connection.close()
...
tables = {
    ...
    CREATE TABLE IF NOT EXISTS Transaction (
        ...
        ALTER TABLE Transaction 
        MODIFY status ENUM('Pending', 'Approved', 'Rejected', 'Borrowed', 'Returned', 'Overdue') NOT NULL;
...
# Step 4: Create tables only if they do not exist
...
cursor.close()
db_connection.close()
#check message for the Database
...
class LibraryApp(ctk.CTk):
    def __init__(self):
        ...
        # Call the Login Screen as the starting screen
        self.login_screen()
    ...
    def login_screen(self):
        ...
        register_button = ctk.CTkButton(self, text="Register", width=130, height=40, corner_radius=10, fg_color="black", hover_color="#8C8477", command=self.registration_screen)
    ...
    def forgot_password_screen(self):
        ...
        verify_button.place(x=200, y=270)
    ...
    def register_user(self):
        ...
        if not all([first_name, last_name, email, address, city, state, zip_code, password, confirm_password]):
        ...
        try:
            ...
            # Insert user
            query = "INSERT INTO User (firstname, lastname, email, address1, city, state, zipcode, password) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"
            cursor.execute(query, (first_name, last_name, email, address, city, state, zip_code, password))
            conn.commit()
    ...
    def login_authentication(self):
        ...
        try:
            ...
            query = "SELECT * FROM User WHERE BINARY email = %s AND BINARY password = %s"
            cursor.execute(query, (email, password))
            user = cursor.fetchone()

            if user:
                self.user_id = user[0]  # Store user ID for session
